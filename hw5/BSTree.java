			...
			if (cmp.compare(currData, target) == 0) {
				...
				if (!ptr.hasLeft()) { //the ptr does not have left child
					...
					if (ptr.hasRight()) { // the ptr has right child
						...
						if (ptr.equals(root)) // the ptr is the root
							root = ptr.right;
						else if (trail.left.equals(ptr)) // ptr is left child
							trail.left = ptr.right;
						else if (trail.right.equals(ptr)) // ptr is right child
							trail.right = ptr.right;
						
					} else { // ptr has child
						
						if (ptr.equals(trail.left))
							trail.left = null;
						else if (ptr.equals(trail.right)) 
							trail.right = null;
					}
				} else {
