...
//				dst.mark();   // not needed because of line 47
	...
	public static void prim(Graph g) {
	    g.toUndirected();
	    PriorityQueue<Edge> pq = new PriorityQueue<>(g.getEComp());
	    Set<Vertex> visited = new HashSet<>();
	    ArrayList<Edge> mst = new ArrayList<>();
	    
	    double sum = 0;
	    Vertex start = g.getVerts().iterator().next();
	    visited.add(start);
	    pq.addAll(start.getAdj().values());
	    
	    while (!pq.isEmpty() && visited.size() < g.nVerts()) {
	        Edge e = pq.poll();
	        Vertex u = e.getSrc();
	        Vertex v = e.getDst();
	        ...
	        Vertex w = visited.contains(u) ? v : u;
	        visited.add(w);
	        mst.add(e);
	        sum += e.getWeight();
	        
	        pq.addAll(w.getAdj().values());
	    }
